...
Post Post::operator+(Post obj) {
	Post tmp; 
	tmp.box = box + obj.box;
	tmp.leter = leter + obj.leter;
	tmp.packet = packet + obj.packet;
	return tmp;
}
Post Post::operator-(Post obj) {
	Post tmp; 
...
int& Post::operator[] (const int index)
{
	return postsIndex[index];
}
void Post::Show(int s) {
	cout << "Posts = <" << s << ">: ";
	cout << " box= " << box << ", leter= " << leter << ", packet= " << packet << endl;
	int m = this->maxConst(box, leter, packet);
	cout << "Max posts = " << m << endl;
}
